...
想说明这个问题或许还是得举个例子，但我也不确定这样的例子是否合适。
...
将空间扩展到2n维之后，延续上面的思考，用$I_{i}$表示守恒量，我们首先想到的问题是

### 在2n维的辛流形上，最多允许有多少个 满足相互对易$\lbrace I_{i},I_{j} \rbrace=0$且$\nabla I_{i}$彼此线性独立的守恒量?

这似乎是一个和辛结构$\omega$以及线性切空间相关的问题。假设允许的守恒量个数用k表示，在2n维辛流形上随意选取点x，我们先考虑x点处的切空间$T_{x}M$. 为了书写简便，令$v_{i}=(\nabla I_{i})\| \_{x}$  s.t. $v_{i}\in T_{x}M$.

这k个向量$v_{i}$张成的子空间用V表示，$V=\text{span} \lbrace v_{1},\dots,v_{k}\rbrace$, 子空间的任意两个元素$v,u$都满足$\omega(v,u)=0$
...
每个守恒量对应着一个约束方程，或者说一个2n维辛流形上的2n-1维的子流形。k个子流形的交最终得到一个2n-k维的子流形。并且满足$\omega(\nabla I_{i},\nabla I_{j})=0$. 当k=n的时候，2n-n=n维的子流形正好是一个拉格朗日子流形，也称为完全可积系统。当 $k<n$ 时，2n-k维子流形的内部也可能包含了一个拉格朗日子流形（前提是需要能找到n个守恒量，这并不总是必然的）。并且这个2n-k维子流形是一个部分可积系统。

我们完全还可以凭着自己的兴趣继续提出各种问题往下探索，随着每一步推演的进行，得到的信息越多，可发散探索的问题通常也越多。但作为一个说明似的举例，到此或许已经足够了。我想试着说明完全没有背景知识，只从一个定义出发，可以推演出什么来。就想我之前常说的，在一个未知且奇异的世界里冒险，自己搭桥，修房子，探索世界。
